...
3. **KVKK aydınlatma metni — SAYFA YAZILDI ve REPODA, ama YAYINDA DEĞİL (2026-08-26).** `gizlilik/index.html` commit'lendi ve uygulamadan Profil ekranındaki link ona gidiyor; **ama sayfa canlıya ÇIKMIYOR.** Sebep bilinçli: `scripts/prepare-public.mjs`'in `gizlilik/`'i `public/`'e kopyalayan değişikliği **commit EDİLMEDİ**. O dosya commit'lendiği an sayfa yayına girer. **Kapıyı açmadan önce şart:** "Yurt dışına aktarımın hukuki dayanağı" başlığı **bilerek boş** — 7499 sayılı Kanun sonrası açık rıza yalnız *arızi* aktarımlar için geçerli, buradaki aktarım *sürekli*, dolayısıyla standart sözleşme gerekebilir. **Avukat onayı gelmeden `prepare-public.mjs` commit'lenmeyecek.** Uygulamadaki **link de `gizli`** (2026-08-26) — sayfa 404'teyken kullanıcıya ölü link göstermemek için. **YAYINA ALMA KONTROL LİSTESİ (üçü BİRLİKTE, tek commit):** ① `scripts/prepare-public.mjs` — yoksa 404 kalır · ② `static/hub.css` (`.belge`/`.metin-tablo`) — yoksa tablo dar ekranda bozulur · ③ `index.html`'de `<a>`'dan `gizli` sınıfını kaldır — yoksa sayfa yayında olur ama linki kimse göremez. **Hesap silme tarafı 2026-08-23'te UÇTAN UCA BİTTİ.** Zincirin tamamı geçti: BLOK 0 → BLOK 4 → BLOK 1 (altı FK `delete_rule=CASCADE`) → edge deploy → UI (`2ecfa41`) → **gerçek hesapla canlı silme, kontrol gruplu** (dokuz satır düştü, dokunulmayan ikinci hesap aynen kaldı). **Aydınlatma metni bilerek en sona bırakılmıştı** — silme uçtan uca çalışmadan metinde "hesabınızı silebilirsiniz" demek yanlış olurdu. **Artık doğru, yazılabilir.** Ayrıntı ve açık kalan 5 madde: üstteki 2026-08-23 bloğu.
