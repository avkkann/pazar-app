...
**Durum: commit edildi, YAYINDA DEĞİL** — fonksiyon **deploy edilmedi** (Mustafa'nın iki adımı aşağıda).
...
**MUSTAFA'NIN İKİ ADIMI:**
1. `supabase secrets set OZET_EPOSTA=<e-posta adresin>` (ya da Dashboard → Edge Functions → Secrets).
2. `supabase functions deploy bildirim-ozet`.
Sonra `update-data.yml`'i elle tetikleyip (`workflow_dispatch`) `bildirim-ozet` job'unun yeşil olduğunu ve e-postanın geldiğini doğrula.
