...
**Son güncelleme:** 2026-09-16 (**"BU FİYAT TUTMADI" BİLDİRİMLERİ ARTIK BİR YERE GİDİYOR** — ölçüldü: `fiyat_bildirim` tablosunu **hiçbir gece işi, hiçbir fonksiyon okumuyordu**; kullanıcı bildirim gönderdiğinde Mustafa'nın haberi olmuyor, veri de düzelmiyordu. Yeni edge function **`bildirim-ozet`** son 24 saatin bildirimlerini (ürün · market · bizdeki fiyat · raftaki fiyat · fark) tek e-postada özetliyor ve gece işine **ayrı job** olarak bağlandı (`needs: update`, HARD-FAIL, `continue-on-error` YOK). **Karar (Mustafa): e-posta bildirim OLMASA DA her gece gidiyor** — gelmemesi "bozuldu" işareti olsun diye. Alıcı adresi **kodda değil** `OZET_EPOSTA` secret'ında (repo PUBLIC); secret yoksa fonksiyon sessizce başarılı dönmüyor, **500** veriyor. E-postada **kullanıcı kimliği yok** (yalnız "kaç farklı kişi"), ürün adı **kaçışlı**, 100 satır sınırı **sessiz değil**. Yeni bekçi **`test_bildirim_ozet.mjs` (60 iddia)** gerçek kaynağı `node:vm`'de sahte Supabase + sahte Resend ile koşturuyor ve gece işine bağlı olduğunu da doğruluyor; prove-by-breaking **11/11**. `sw.js` **v250 SABİT** (backend-only). **CANLI ve uçtan uca doğrulandı (2026-09-16):** secret eklendi, fonksiyon deploy edildi, gece işi elle tetiklendi ([run 35069949979](https://github.com/avkkann/pazar-app/actions/runs/35069949979)) — üç job da yeşil, `bildirim-ozet` **HTTP 200** `{"ok":true,"bildirim":1,"urun":1,"kisi":1,"kirpilan":0}` döndü ve **e-posta Mustafa'ya ulaştı**. Öncesi — 2026-09-14 (**MASAÜSTÜ FIRSATLAR SEKME ÇUBUĞU OKUNUR OLDU** — ≥1024px'te toplu bir kural (`.search-wrap, …, .firsat-tabs { background: transparent }`) çubuğu saydam yapıyordu, çubuk yeşil bandın üstüne düşüp gri/koyu yeşil sekme yazıları okunmuyordu. Çubuk artık kendi yeşil zeminini (`--zemin-a`) taşıyor, yazılar beyaz (aktif 10,35 · pasif 7,28); yalnız yazıyı beyaz yapmak yetmezdi çünkü çubuk sticky ve kaydırınca arkasına açık liste zemini geliyor. Mobil ve koyu tema değişmedi (ölçüldü). Yeni bekçi `test_firsat_sekme_masaustu.mjs` (21 iddia, CSS basamağını çözerek), prove-by-breaking **7/7**, `sw.js` **v250**. Öncesi — 2026-09-13 (**FİYAT UYUŞMAZLIĞI: KAYNAK TEK BİR İSTANBUL MAĞAZASININ FİYATINI VERİYOR — ARTIK "NEREDEN" YAZIYOR** — Mustafa CarrefourSA'nın kendi uygulamasında **449,90 (kart) / 749,90** görürken bizde **399,90** yazıyordu. Ölçüldü: bu ayrıştırma hatası **DEĞİL**, **kapsam** hatası — marketfiyati her zincir için **TEK temsilci mağaza** döndürüyor ve katalogu besleyen **27 mağazanın HEPSİ İstanbul'da** (Kadıköy/Üsküdar/Ümraniye/Acıbadem); o ürün Antalya mağazalarıyla sorulduğunda kaynakta **hiç yok**. Örneklem (40 ürün): **%40'ı Antalya'da yok**, bulunan 24'ün 22'sinde fiyat birebir aynı — ama risk **indirim iddialarında** yoğunlaşıyor: canlı düşenler listesinden 25 iddianın **3'te 1'i orada geçersiz** (%12 ürün yok, %20 fiyat farklı; Familia Havlu İstanbul **99,90** ↔ Alanya **174,95**, Omo Express BİM **299** ↔ Alanya **469**). `indexTime` örneklemin tamamında **bugün** → sorun bayatlık değil, **konum**. **D + A yapıldı:** API'nin verip **attığımız** `indexTime` artık `fiyat_guncelleme`, mağaza koordinatından türeyen il artık `depot_il` olarak additive kaydediliyor (il tablosu **`iller_tablo.py`'de TEK KAYNAK**, `il_market_tara.py` de oradan okuyor); ürün detayında fiyatın **hangi mağazadan/ilden ve ne zaman** geldiği yazıyor; ana sayfadaki tazelik satırına **veriden türeyen** kapsam cümlesi eklendi ("· 27 mağazadan derlendi" — il listesi yarınki gece koşusundan sonra dolacak, sabit şehir YAZILMIYOR); üstü çizili "eski fiyat" bizim gözlemimizi aşıyorsa artık **"-%N" rozetiyle sunulmuyor** ("ilan · biz görmedik") — ölçüldü: ilanların **%27'si (749/2776)** bizim o markette hiç görmediğimiz bir değer ve kaynak o kayıtlarda `discount:false` diyor. `test_depot.py` 22 → **30**, yeni bekçi **`test_fiyat_kapsami.mjs`**, prove-by-breaking **13/13**. **B — "her şehrin fiyatı" ÖLÇÜLDÜ ve ERTELENDİ:** 8 il × indirim iddialarında başarısızlığın 28/33'ü fiyat farkı değil **"ürün o ilde kaynakta yok"** — bölgesel çekim bunu çözmüyor; öncelik **marketlerden doğrudan veri** (ayrıntı aşağıdaki bölümde). Öncesi — 2026-09-12 (**ŞERİT 12 KART + "TÜMÜNÜ GÖR" + FIRSATLAR'DA 4. SEKME: "BU HAFTA DÜŞENLER"** — şerit 6 → **12 kart**; market sınırı 2 → 3 (ölçüldü, 336 ürünlük havuz: 12. kart ≤2'de havuzun **106. sırasından** -%25, ≤3'te **50. sırasından** -%31, ≤4'te 4 market kalıyor). Şeridin yanında **"Tümünü gör"**, havuzun **TAMAMI (336 ürün)** Fırsatlar'da yeni sekmede; liste build'de şeritle **AYNI havuzdan** üretiliyor (`data/dusenler.json`, 142,5 KB ham / **23,3 KB gzip**, tembel — yalnız sekme açılınca iniyor, sw önbelleğinde değil, depoya girmiyor). Salt-okunur **düşmanca inceleme** (17 ajan, 20 bulgu → 6 doğrulama → **5 ayakta**, 1 çürütüldü) beşini de düzelttirdi: **(a)** veri hattında "ölçülemedi" ile "ölçüldü, sıfır" ayrımı YOKTU — build sırasında Supabase çökerse havuz boş yazılıyor, dört şeridin birden boş olmasını isteyen emniyet kapısı hiç tetiklenmiyor ve yeni sekme (şeritten farklı olarak gizlenmiyor) **"0 Düşen ürün"** diye ölçülmüş gibi duran yanlış bir iddia basıyordu → veri artık `olculdu` bayrağı taşıyor, ölçülemediğinde sayı **yazılmıyor**; **(b)** kartta **ÜÇ kaynak** karışıyordu (rozet ve "normal" anlık görüntüden, indirimli fiyat CANLI katalogdan) → bayat katalogla "-%60" derken alt metin "249,90 ₺ → 249,90 ₺" diyebiliyordu; `dusenKayit`'e `fiyat` eklendi, kart **tek kayıttan** çiziliyor; **(c)** kart **kararsız `_id`** ile çözülüyordu (gece yeniden dizilince aynı index başka ürüne denk gelir); **(d)** özet çipi sekme değişince temizlenmiyordu ("En Ucuz" sekmesinde "336 Düşen ürün"); **(e)** sekme çubuğu kaydırma yardımcısı sahte DOM'da **sessizce ölüydü** (test körlüğü). `test_dusenler.mjs` 136 → **198 iddia**, prove-by-breaking **31/31**, `sw.js` **v248**. **Aletin aleti iki kez yanılttı:** bozma planında `"test": "node test_dusenler.mjs"` yazmak aracın "test zaten KIRMIZI" demesine yol açtı (araç dosya adı bekliyor) ve iki çapa tekil değildi → mutasyon YANLIŞ yere uygulanıp "guard kör" gibi okundu; artık `scripts/`+scratchpad'de çapa sayan bir ön kontrol var. Öncesi — 2026-09-11 (**"BU HAFTA DÜŞENLER" İKİNCİ İNCELEME — DÜZELTMELER DÜŞMANCA DOĞRULANDI** — üç bakış açısı + her bulguya ayrı çürütme denemesi: **12 bulgu doğrulandı, 0 çürütüldü**, hepsi aynı sürümde düzeltildi. En önemlileri: 1-3 günlük bir kesinti haftalardır süren indirimi "bu hafta" gösteriyordu (3 günlük köprü); teyit artık derin fiyatın kaç gündür görüldüğüne ve marketin KENDİ düşüşüne bakıyor; başka markette haftalardır duran ucuz fiyat ürün referansına giriyor; **tarayıcıda şüphe puanları 1000'de kesiliyordu** (1431 satırın 431'i inmiyordu — bu düşenlerden ÖNCE de vardı), artık sayfalı ve gerçek sunucuda 1431/1431. `test_dusenler.mjs` 104 → **136**, prove-by-breaking **49/49**. Öncesi aynı gün — **"BU HAFTA DÜŞENLER" İNCELEME TURU — ROZET ARTIK ABARTMIYOR** — yayındaki ilk sürümü iki salt-okunur ajan inceledi, iddiaları gerçek veriyle ölçüldü. Rozet market kendi normaline göre ölçülüyor, kart ise tüm marketlerin en ucuzunu gösteriyordu: Dalin "-%51" → gerçek **%11**, o günkü 6 kartın **3'ü** yanıltıcıydı → rozet artık iki ölçünün KÜÇÜĞÜ (ürünün olağan en ucuzu yalnız BUGÜN satan marketlerden — çıkan marketin eski fiyatı carry-forward ile sonsuza taşınıyordu: Toblerone "75 TL"). Şüphe puanları alınamazsa liste çıkmıyor (**321 → 568**, şüpheliler sessizce sızıyordu); eleme ham puana bakıyor; pencere verinin gününe bağlı (73 ürün bir gün erken düşüyordu); ≥%50 indirim 2 gün teyit istiyor (tek günlük derin düşüşler kümeleniyor); build ve istemci tek kayıt biçimi (`dusenKayit`). `test_dusenler.mjs` 68 → **104**, prove-by-breaking **29/29**, `sw.js` **v247**. Öncesi aynı gün — **"BU HAFTA DÜŞENLER" YENİDEN KURULDU** — şerit artık Supabase RPC'sinden değil, marketin kendi 60 günlük normal fiyatına göre SON 7 GÜNDE başlayan ve bugün süren indirimlerden kuruluyor. Eski ölçütün 40 adayından 28'i hatalı referanslıydı (12 Ağustos Hakmar tek günlük hatası 14 ürünü 28 gün "-%95" gösterdi), görünen 6 kartın yalnız 1'i o hafta düşmüştü; yeni havuz **693 ürün**. İlan fiyatı yalnız gözlemle uyuşuyorsa kanıt; kart fiyatı = indirimli fiyat; çeşitlilik marka≤1 · kategori≤2 · **market≤2** (ölçüldü: aksi hâlde 6'nın 6'sı Carrefour); rozet marketi söylüyor; çeşitlilik döngüsü zam şeridiyle ortak (`_cesitliSec`). `test_dusenler.mjs` 68 iddia, prove-by-breaking **15/15** — biri kendi kör noktamı buldu. `sw.js` **v246**. Öncesi — 2026-09-10 (**`marka` ALANI KAYDEDİLİYOR + YORUM SOYUCUSU İKİNCİ KEZ ONARILDI** — API `brand`'i 16.434 başlığın %100'ünde veriyordu ve atıyorduk; marka, ad dışında sahip olduğumuz TEK ayırt edici alan (resimsiz 2.384 ürünün %92,7'si markalı paketli). Additive, `"Markasız"` yazılmıyor (479 ürün), koruma kuralı resimle TEK KAYNAKTA (`_alan_koru`). Canlı API: 75 üründen 52'sine yazıldı, yazılmayan 23'ün 23'ü de API'de "Markasız". **Görsel çekme hattı KURULMUYOR — karar Mustafa'nın:** gözlem %92,7 doğru ama dürüst tavan açığın ~%15'i (Carrefour/Migros/A101 sözleşmeyle yasaklıyor = %52; BİM'in kataloğu yok = %22,7) ve baskın risk YANLIŞ fotoğraf. **Yorum soyucusunun "güvenli" sürümü de bozuktu:** JS'te `.` `\r`'yi eşlemiyor, `m` yokken `$` dize sonu bekliyor → CRLF satırda yorum HİÇ soyulmuyordu; CI Linux'ta LF olduğu için orada yeşil, yalnız Windows'ta kırmızı. Regex yerine karakter koduyla onarıldı ve **soyucuya beş dosyada kendi kontrol grubu gömüldü** (aynı ders iki tur önce yazılmış ama uygulanmamıştı). Prove-by-breaking **5/5** + `test_marka.py` 10/10. `sw.js` **v245 SABİT** — backend/test değişikliği. Öncesi — **YÜKSEK grubunun KALAN 4 GRUBU BİTTİ** — 12 bulgu tek turda: koyu temada seçili sekme 1,17 → 15,25 ve giriş paneli 1,24 → 12,32; tarayıcı/Android geri tuşu artık uygulamadan ÇIKARMIYOR (history.length ölçüldü: 1 → 1 idi); hızlı kategori geçişinde yanlış liste (başlık "Temizlik", liste 779 ET ürünü) kapandı; damgalı JS/CSS `immutable`; kapalı panellerde 9 hayalet düğme → 0; sıralama menüsü klavyeyle kullanılabilir; ana sayfada ürüne dokunmak 19.742 KB → 8.357 KB; Türkçe ekler kurala bağlandı; `scraper.py` ağ koparsa yarım dosyayı ARTIK üzerine yazmıyor + beş yazma yeri atomik; `test_iddia.mjs`'in koşulsuz 3 PASS'ı kalktı. Prove-by-breaking **69/69**; üç bekçi kendi kör noktasını açık etti ve sıkılaştırıldı; **kendi yorum soyucum bozuktu** — dosyanın %55'ini siliyordu, güvenli sürümle değişti. **⑨ görsel boyutu ölçüldü ama KARAR MUSTAFA'DA** — Cloudflare Image Resizing bir hesap kararı. `sw.js` **v245**. Öncesi — 2026-08-31: **KVKK AYDINLATMA METNİ YAYINDA** — `/gizlilik/` canlı, Profil'deki link görünür; kontrol listesinin üçü de tek commit'te gitti. Boş duran "yurt dışına aktarımın hukuki dayanağı" bölümü yayına alınırken **sayfadan çıkarıldı**, avukat görüşünden sonra geri gelecek — taslak metin `2e69630`'da. `.metin-tablo` kuralının yük taşıdığı prove-by-breaking ile ölçüldü: kural silinince tablo 383px → **1694px**. Öncesi — 2026-08-26: metin repoya alınmıştı ama yayında değildi. Ölçüm düzeltmesi: push bildiriminde ürün adı/hedef fiyat YOK ve yük uçtan uca şifreli. Öncesi — 2026-08-25: **SEKME GEÇİŞİ: İKİ EKRAN BİRLİKTE KAYIYOR** — ilk çözüm boş kareyi ötelemeyi 100%→16px kısarak kapatmıştı, geçiş algılanamaz oldu; DOĞRUSU giden ekranı GÖRÜNÜR tutmak. Şimdi tam genişlik (390px), iki ekran birden 15 kare, boş kare 0, yatay taşma 0 (gizli kusur: `overflow-x: clip` absolute öğeyi kırpmıyordu → `body { position: relative }`), CLS 0. `test_gecis.mjs` 72 iddia, prove-by-breaking 18/18. Öncesi — **GEÇİŞLER YUMUŞATILDI** — sekme geçişinde ekranın ~35-41ms BOŞ kaldığı ölçüldü (ilk kare kapsamı %0) ve sıfırlandı; splash sönmesinde kusur SÜREDE DEĞİL EĞRİDEYDİ (%83'ü ilk 50ms'de bitiyordu → linear). reduced-motion'da animasyon TAM kapalı, layout birebir aynı. `test_gecis.mjs` 41 iddia, prove-by-breaking 12/12. Öncesi aynı gün — **FİYAT GRAFİĞİ ETİKET ÇAKIŞMASI KAPANDI** — E-E/E-X/X-X/TAŞMA **sıfırlandı**, etiket↔çizgi hale ile okunur; 193 grafiğin tamamı gerçek tarayıcıda sayıldı, çakışma "karışıklık" değil **yanlış fiyat okuma** üretiyormuş: "103,95 ₺" ekranda "105,95 ₺" gibi okunuyordu. `test_fiyat_grafik.mjs` 51 iddia, prove-by-breaking 13/13. Ayrıntı: aşağıdaki "Madde 1 KAPANDI" bloğu. Öncesi — 2026-08-23: **KVKK HESAP SİLME UÇTAN UCA CANLI** — cascade FK + `hesap-sil` edge function + iki adımlı onaylı UI (`2ecfa41`); gerçek hesapla kontrol gruplu doğrulandı, kalan tek halka **KVKK aydınlatma metni**. Öncesi: BLOK 4 + BLOK 1 geçti, altı FK `ON DELETE CASCADE`, `sql/` repoya alındı; **`style-src`'den `'unsafe-inline'` kaldırıldı**, `script-src` bilinçli ertelendi + satır içi handler kilidi). Öncesi — 2026-08-21/22 oturumu (**güvenlik başlıkları** — `font-src 'self'` + `frame-ancestors 'none'` + nosniff, **HSTS** 1. basamak `max-age=300` → **2026-08-22'de 2. basamak `max-age=86400`**; **CI test kapısı** — `deploy needs: test`, 50 test (glob), kasıtlı FAIL ile kanıtlandı; **B1 kaçış** dört kaçışsız nokta kapatıldı (`test_kacis` 93 iddia); GITHUB_TOKEN varsayılanı read + workflow başına açık `permissions`; Grup 1 (M1/M2/M3/M4); **`www` → apex 301 kuruldu**, CF beacon panelden kapatıldı; fontlar self-host + GoatCounter pin; **KVKK / hesap silme DEVAM EDİYOR** — ölçüm bitti, taslaklar henüz çalıştırılmadı; **sw v232**). Ayrıntı için aşağıdaki "2026-08-21" blokları. Bu dosya her oturum başında okunur, sohbete asla ham metin olarak yapıştırılmaz.
...
### 2026-09-11 — "Bu hafta düşenler" YENİDEN KURULDU: gerçek haftalık indirimler (`sw.js` v245 → **v246**)

**Durum: yayına alındı (2026-09-11, Mustafa "canlıya al" dedi).** Mustafa'nın gözlemi: "2 haftadır indirimi biten ürünler duruyor, yeni ürün girmiyor, biri indirimde bile değil."

**Kök sebep (ölçüldü 2026-09-10).** Şerit Supabase'deki `get_fiyat_dusenler` RPC'sinden geliyordu. **SQL'i depoda YOK**; davranışı ölçümle çıkarıldı: indirim, ürünün en az 30 günlük (büyük ihtimalle tüm geçmişin) **en yüksek fiyatına** göre hesaplanıyordu — son 7 günün en yüksek fiyatı 40 adayın yalnız 10'unda tutuyordu. Zaman penceresi yoktu.

| | ölçüm |
|---|---|
| referansı hatalı aday | **28/40** (ürünün diğer fiyatlarının medyanının 2 katından fazla) |
| en büyük tek kaynak | 12 Ağustos'ta Hakmar'da tek günlük hata (35 g kek 149 TL) → **14 ürün 28 gün "-%83…-95"** |
| görünen 6 kartta bu hafta gerçekten düşen | **1** |
| listenin 40. sırası | **-%58** — gerçek haftalık indirimlerin %94'ü (%10–49) listeye hiç giremiyordu |
| aynı kampanya | 6 kartın 3'ü aynı A101 Kent Jelibon |

> "2 haftada yalnız 1 yeni ürün" sayıca TUTMADI (o günkü 6 kartın 5'i son 14 günde girmişti) ama hissi doğruydu: girenler hep aynı türdendi — hatalı referans ya da aynı kampanyanın kopyası.

**Yeni ölçüt — app.js'te; build de istemci de AYNI kodu çağırıyor** (`dusenOlcutu` saf/tek market · `dusenHavuzu` katalog · `dusenSecHavuzdan` seçim). Build betiğinde eşik yok:
1. **Referans = marketin normal fiyatı:** bu haftadan önceki 60 günün günlük (carry-forward) serisinin **üst ortancası**. Tek günlük hatalı zirve ortancayı kıpırdatamaz; üst ortanca her zaman gerçekten görülmüş bir fiyat.
2. İndirim **son 7 günde başlamış**, öncesinde indirimsiz gün **görülmüş**, **bugün sürüyor**, en az **%10**. Biten ya da 7 günden eski indirim kendiliğinden düşer (biten kampanyaların %65'i 4–14 gün sürüyor — ölçüldü).
3. **İlan edilen eski fiyat** (`liste_fiyat`) yalnız geçmiş kısaysa (<30 bilinen gün) kanıt ve yalnız **indirimden önce gözlenen fiyatla uyuşuyorsa** (gözlenen fiyat ilanın altındaysa indirim koşusu 7 günden eskiye uzar, üstündeyse ±%5 kapısı eler). Geçmiş yeterliyse ilan referansı **ezmez**: Migros normali ~170 TL olan bir deodorantı "eski fiyat 329,95" diye gösteriyor (ölçüldü). Bu yol haftada **42** ürün ekledi.
4. **Kart fiyatı = indirimli fiyat:** indirimli market kartın gösterdiği en ucuz fiyatı vermiyorsa ürün girmez (kaybı ölçüldü: 53 kayıt).
5. **Çeşitlilik marka≤1 · alt kategori≤2 · market≤2**, 6 kart. **Market sınırı ölçümle eklendi:** yalnız marka/kategori kuralıyla ilk 6'nın **6'sı Carrefour** çıkıyordu. Döngü `_cesitliSec` — **zam şeridiyle ortak** (zamSecHavuzdan da ona geçti; iki kopya = sapma); kural yüzünden dolmazsa eşik düşürülmez.
- **Rozet artık marketi söylüyor** ("-%60 CarrefourSA"): indirim market bazlı ölçüldüğü için marketi söylemeyen yüzde belirsiz kalırdı. Sınıf zam rozetininkiyle ortak (`.zam-rozet-mkt`), yeni CSS yok.
- Hazır liste boşsa ("bu hafta düşen yok") şerit gizlenir; istemci 14 MB katalog indirip yeniden hesaplamaz. Geriye düşüş (anasayfa.json yok) zam şeridiyle aynı yol.

**Ölçüm (2026-09-10 verisi):** havuz **693 ürün** (651 geçmişten + 42 ilandan, örtüşme yok) — eski yolda 40 aday. %70 üstü indirim **0** → ayrı üst sınır gerekmedi. Katalog taraması ~3,2 sn (build). Build'in seçtiği 6 kart: 2 CarrefourSA, 2 Migros, 2 A101; %41–60; hepsi bu hafta başlamış.

**Eski RPC EMEKLİ:** ne istemci ne build çağırıyor, `DUSENLER_RPC_LIMIT` silindi. Fonksiyon Supabase'de duruyor — kaldırmak Mustafa'nın kararı (önce `select pg_get_functiondef('public.get_fiyat_dusenler'::regproc);` ile tanımı arşivle).

**Bilerek YAPILMAYAN (not düşülüyor):** (a) şehir filtresi düşenlere uygulanmadı — bugün de yoktu ve Profil'deki "bu seçim neyi etkiler" cümlesini değiştirir, ürün kararı. (b) Ürün detayındaki "Son ayın zirvesinden %X ucuz" rozeti (`indirimRozetiHesapla`) **dokunulmadı** — o da 30 günlük seride tek günlük zirveye duyarlı (Tamek Kayısı Nektarı vakası), ayrı iş. (c) Gece işinin geç başlaması (son 2 haftada TR 10:34–12:56, 3 Eylül koşusu başarısız) GitHub'ın zamanlayıcısı; dokunulmadı.

> **BOZMA TESTİ KENDİ KÖR NOKTAMI BULDU.** 14 mutasyonun 13'ü kırmızıya döndü; "ilan fiyatı gözlemle uyuşmalı" kapısı silinince test **yeşil kaldı**. Sebep: şişirilmiş ilan örneğini 7 gün penceresi de eliyordu. Kapının tek başına belirleyici olduğu yön (gözlenen fiyat ilanın ÜSTÜNDE) teste eklendi → 1/1 kırmızı. Ayrıca `dusenOlcutu`'da hiçbir durumda tetiklenemeyen bir satır vardı (iki kapı zaten zorunlu kılıyordu) — silindi, yerine neden garanti olduğunu anlatan yorum kondu.

> **Üç test aynı sebeple ÇÖKTÜ, üçü de GERÇEK kaynakla onarıldı:** `test_zam`, `test_anasayfa`, `test_salinim` zamSecHavuzdan'ı kendi vm'lerinde koşuyor → `_cesitliSec is not defined`. Sahte değil gerçek `fnKaynak('_cesitliSec')` yüklendi. `test_cekirdek` de haklı olarak kırmızıydı: çekirdek listesine `_cesitliSec` eklendi, `core/pazar-cekirdek.cjs` yeniden üretildi.

> **test_supheli'nin iki düşenler testi TAŞINDI, gevşetilmedi.** Eski RPC'yi taklit ediyorlardı (`rpcLimit > 6`). Aynı üç niyet (şüpheli girmez / üst adaylar şüpheli olsa da şerit 6 dolar / hepsi şüpheliyse gizli) `test_dusenler.mjs`'te gerçek app.js ile kilitli; "RPC ÇAĞRILMIYOR" iddiası eskisinin tersi.

> **Ölçüm aleti notu:** tarayıcı panosunun ekran görüntüsü kaydırılmış bölgeyi göstermedi (sayfa `innerHeight` 720 derken görüntü 455 yüksekliğinde, üstü boş kare verdi). Kanıt DOM ölçümünden alındı.

**Yeni bekçi:** `test_dusenler.mjs` (**68 iddia**) — saf ölçüt (hatalı zirve, eski/bitmiş indirim, pencere sınırları, ilan iki yön, üst ortanca), havuz (en ucuz market, tekilleştirme, şüphe, sıralama), seçim (çeşitlilik, eşik düşürülmez), çizim (hazır liste / boş liste / kaçış / geriye düşüş, RPC çağrılmıyor) ve **gerçek veri** ("bugün" = verinin son günü; her kayıt kurala uyuyor, 6 kart doluyor). Prove-by-breaking **15/15** + ortak çeşitlilik zam tarafında **2/2**.
**Doğrulama:** 66 `test_*.mjs` (**2.766 iddia**) + 8 `test_*.py` yeşil · build yeşil · yerel `dist`'te tarayıcı: şerit görünür, 6 kart, rozetlerde market adı, önbellek v246, konsol hatası 0.

#### Aynı gün, İNCELEME TURU — iki ajanın bulguları ölçülüp düzeltildi (`sw.js` v246 → **v247**)

**Durum: yayına alındı (2026-09-11, Mustafa "bitince canlıya al" dedi).** Yayındaki ilk sürümü iki salt-okunur ajan inceledi (kod + testlerin kör noktaları). İddiaları olduğu gibi kabul edilmedi, **gerçek veriyle ölçüldü** — neredeyse hepsi doğru çıktı:

| | Bulgu | Ölçüm | Düzeltme |
|---|---|---|---|
| a | Şüphe puanları alınamazsa şüpheliler **sessizce** giriyordu | havuz **321 → 568** (Supabase çökünce 247 şüpheli ürün) | puan yoksa liste çıkmaz + uyarı; `supheliPuanlariYukle`'nin sessiz hata dalına da uyarı |
| b | Eleme başka bir indirim tanımına bağlıydı: `supheliDurum` 30 günlük "en ucuz" seride indirim görmezse null | havuzda ham puanı ≥2 olan **11/394** (biri "kutu" seviyesi, puan 5) | ham puana bakılıyor; `DUSEN_SUPHE_PUAN` supheliDurum'un eşiğiyle testle kilitli |
| c | **Rozet abartıyordu — canlıda:** market kendi normaline göre ölçülüyor, kart ise tüm marketlerin en ucuzunu gösteriyordu | Dalin "-%51" → gerçek **%11**; o günkü 6 kartın **3'ü** yanıltıcı; havuzun 25'inde ürünün en ucuz fiyatı %10 bile düşmemişti | rozet = min(marketin kendi normaline göre, **ürünün olağan en ucuzuna** göre) |
| d | 7 günlük pencere duvar saatine bağlıydı | **73 ürün** bir gün erken düşüyordu | varsayılan "bugün" = verinin son günü (bugünle sınırlı) |
| e | Tek günlük hatalı DÜŞÜK fiyata karşı koruma yoktu | geçmişte tek günlük ≥%40 düşüş **226** (120 günde); en büyük üç küme toplamın **%31**'i (22 Haziran'da Migros'ta 42 ürün aynı gün) | gösterilen ≥%50 ise (indirim_analiz.py'nin "aşırı yüksek oran" sınırı, testle kilitli) en az **2 gün** görülmeli |
| f | Build ile istemcinin alan adları ayrı yerdeydi | — | tek biçim: `dusenKayit` |

> **"ÜRÜNÜN OLAĞAN EN UCUZU" TANIMINDA BİR TUZAK ÇIKTI.** Geçmişte "market çıktı" kaydı yok; carry-forward, artık satmayan bir marketin son fiyatını sonsuza taşıyor. Toblerone'da "olağan 75 TL" çıkıyordu; yalnız **bugün satan** marketler sayılınca 179,90. *Aynı sınıf etki `_seriKur`/zam tarafında da olabilir — ölçülmedi, ayrı iş.*

> **Neden iki ölçünün KÜÇÜĞÜ:** ikisi de tek başına abartabilir. Ürün düzeyi, market kısa süredir satıyorsa marketin kendi ilanından büyük çıkabiliyor (Omo: ürün düzeyi %50, Migros'un kendi ilanı %40). Rozet ikisinin de desteklediği kadarını söylüyor.

**Test kör noktaları (ikinci ajan), hepsi kapandı:** karta dokununca detay açılmasını sağlayan `productMap` kaydı hiç ölçülmüyordu · TL eşitlik bozma, testteki katalog sırası beklenen sırayla aynı olduğu için ölçülmüyordu · ilan yolu havuz düzeyinde test edilmiyordu · 60/30/±5/7 sınırları sabitlenmemişti · **taşıdığım "gizle" testini ben gevşetmiştim** (şerit zaten gizli başlıyordu, gizleme satırı silinse de yeşildi) · gerçek veride "tam 6 kart" beklentisi yanlış kırmızı verebilirdi → yerine **bağımsız bir hesapla birebir karşılaştırma**.

**Sonuç (2026-09-10 verisi, gerçek şüphe puanlarıyla):** Nar 1 Kg -%57 CarrefourSA · Finish Quantum Max -%53 CarrefourSA · Hellmann's Acılı Mayonez -%52 Migros · Pınar Ton Balığı -%49 Migros · Kotex Gece Ped -%41 A101 · Bingo Renk Koruma -%39 Tarım Kredi. Katalog taraması **~3,2 sn → ~0,25 sn** (pencerenin gün listesi bir kez kuruluyor).

**Bilerek YAPILMAYAN (ek):** meyve/sebzenin mevsimsel düşüşleri şeride girebiliyor (Nar -%57); zam şeridindeki "mevsim tuzağı" elemesi burada uygulanmadı — alışveriş açısından gerçek düşüş, ürün kararı.

**Doğrulama:** `test_dusenler.mjs` 68 → **104 iddia**, TDD (önce **16 kırmızı**, hepsi doğru sebepten) · prove-by-breaking **29/29** · 66 `test_*.mjs` (**2.802 iddia**) + 8 `test_*.py` yeşil · build yeşil.

#### Aynı gün, İKİNCİ İNCELEME — düzeltmelerin düşmanca doğrulanması (aynı sürüm, `sw.js` v247)

Düzeltmelerin kendisi bu kez **üç bakış açısıyla** (doğruluk, dayanıklılık, test yeterliliği) incelendi ve her bulguyu **ayrı bir doğrulayıcı çürütmeye çalıştı**: **12 bulgu doğrulandı, 0 çürütüldü**; kotaya sığmayan 6 bulgu doğrulanmadan döndü, onları testle kendim doğruladım. Hepsi düzeltildi ve ilk incelemenin düzeltmeleriyle **aynı sürümde** yayına gidiyor.

| | Bulgu | Ölçüm | Düzeltme |
|---|---|---|---|
| g | 1-3 günlük bir kesinti, haftalardır süren indirimi "bu hafta başladı" gösteriyordu | Mowi somon burger 26 gündür indirimdeydi, tek günlük 269,95 sıfırladı; 88 günlük geri oynatmada seçilen kartlara **29 giriş** (aralar 1 gün 10, 2 gün 13, 3 gün 6); bir kısmı mağaza değişiminden gelen hayalet gün (Wasa 1012 → 5411) | en fazla **3 günlük** ara, öncesinde yine indirim varsa köprülenir (`DUSEN_KESINTI_GUN`) |
| h | Teyit koşunun BAŞINA bakıyordu (dün %4'lük adım, bugün %60 teyitsiz geçiyordu) ve gösterilen küçük yüzdeye bakıyordu (Migros 100 → 20 tek gün, BİM 36 varken "-%44") | gerçek geçmişte Mutlu Arpa Şehriye, Tadıbu | teyit, **derin fiyatın kaç gündür görüldüğüne** (`seviyeGun`) ve **marketin kendi düşüşüne** bakıyor |
| i | Başka markette haftalardır duran aynı ucuz fiyat ürün referansında görünmüyordu (ortanca, pencerenin yarısından kısa süreni yok sayıyor) | BİM 34 güne kadar 60 TL satarken Migros'un 60'ı "-%40" | ürün referansına **başka marketlerin indirimden önceki son 7 gündeki en ucuzu** da giriyor, küçüğü alınıyor |
| j | Aykırı fiyat filtresi, ürünün olağan fiyatını kanıtlayan marketi referanstan çıkarıyordu | Migros 100 → 11, BİM hep 56 → "-%89" (doğrusu %80) | referans HAM market listesinden |
| k | **Tarayıcıda şüphe puanları 1000'de kesiliyordu** | puanı ≥2 **1431** satır; sayfasız istekte 431'i inmiyordu. Build'in vm katmanı kendisi sayfaladığı için build'de görünmüyordu — **bu kusur düşenlerden ÖNCE de vardı**, tarayıcıdaki şüpheli-indirim rozetlerini de etkiliyordu | `.order('_sid').range()` ile sayfalı (vm katmanına `range` eklendi); sunucu aralığı dinlemezse 50 sayfada durur ve yarım liste KULLANILMAZ. Gerçek sunucuda ölçüldü: **1431/1431** |
| l | Build sırasında Supabase çökerse şerit bir gün sessizce boş kalıyordu, CI yeşil | — | iki ek deneme + CI'da görünür `::warning` |
| m | Tek bir saatli tarih kaydı ("…T08:00") bütün şeridi uyarısız boşaltıyordu | pencerenin 67 günü "NaN-NaN-NaN" | veri günü yalnız `yyyy-aa-gg` kabul ediyor |
| n | Geçmişte dize fiyat alfabetik karşılaştırılıyordu ("90" < "100" yanlış) | bugün tetiklenmiyor | `_gunlukSeri` sayıya çeviriyor |

**Test kör noktaları da kapandı:** şüphe puanı **tam 2** (598 ürünlük en kalabalık grup) · build'in puanları **bekleyerek** ve düşenlerden **önce** yüklemesi · marketler arası **en ucuzun** kayıt sırasından bağımsız seçilmesi · **üst ortanca** · **yuvarlama sınırları** (%9,9 girmez; kayan noktada tam %10 girer; %49,6 → %50 teyit ister) · eşit yüzdede **market seçimi sıraya bağlı değil** (derin fiyatı daha uzun süren seçilir) · TL eşitliği **gösterilen** referansa göre · `supheliPuanlariYukle` uyarısı · ürün referansındaki 7 günlük kapı **hiç tetiklenemiyordu** (ölü koşul silindi, yerine neden garanti olduğunu anlatan yorum).

> **"Yakın alternatif" kuralı kendi testimi geçersiz kıldı.** İlk turdaki "ham puan" testinin vakası (BİM 30 gündür 80) tam da bu kuralın yakaladığı desendi; vaka artık şüphe elemesini ölçmüyordu. Test gevşetilmedi, **ölçtüğü şeyi yeniden ölçecek biçimde kuruldu**: şüphe denetiminin gerçekten kaçabildiği yol, artık satmayan bir marketin eski ucuz fiyatının `supheliDurum`'un 30 günlük serisinde kalması. Aynı sebeple iki Dalin testi 2 günlük indirimle kuruldu — marketin kendi düşüşü tam %50 olduğu için (h) teyidi ilk günü zaten eliyor; o testler ÜRÜN düzeyini ölçüyor.

**Sonuç (2026-09-10 verisi):** havuz **548** ürün (test ortamında, şüphesiz; ilk turda 626). **Bugünkü 6 kart değişmedi** — yeni kurallar ilk sıradakileri değil, havuzun gövdesindeki hatalı kayıtları eledi. Build gerçek Supabase'ten sayfalı yoldan puan aldı, uyarı yok.

**Doğrulama:** `test_dusenler.mjs` 104 → **136 iddia**, TDD (önce **16 kırmızı**) · prove-by-breaking **49/49** (yeni ve değişen 25 + önceki turun 24'ü yeni test dosyasına karşı) · 66 `test_*.mjs` (**2.834 iddia**) + 8 `test_*.py` yeşil · build yeşil · yerel `dist`'te tarayıcı: 6 kart, rozetlerde market, karta dokununca doğru detay, önbellek v247, konsol hatası 0.

---

### 2026-09-16 — "Bu fiyat tutmadı" bildirimleri için gecelik e-posta özeti (`sw.js` v250 SABİT)

**Durum: CANLI (2026-09-16).** Fonksiyon deploy edildi, `OZET_EPOSTA` secret'ı tanımlandı, gece işi elle tetiklenip doğrulandı: üç job yeşil, özet adımı **HTTP 200** ve e-posta geldi. Kurulum adımları aşağıda (tekrar gerekirse) tarihsel kayıt olarak duruyor.

**Kök tespit (ölçüldü, 2026-09-16).** Mustafa sordu: *"bu fiyat tutmadı bildirimleri nereye gidiyor?"* Kod izlendi: bildirim `app.js`'ten `fiyat_bildirim` tablosuna tek satır olarak düşüyor (`_sid`, `market`, `gosterilen_fiyat`, `bildirilen_fiyat`, `kullanici_id`, `olusturma`) ve **orada kalıyordu**. Tabloyu okuyan tek şey `get_fiyat_bildirimleri` (ürün detayındaki "N kişi bildirdi" uyarısı) ve `get_kendi_bildirim_sayim` (profil). **Hiçbir gece işi, hiçbir edge function okumuyordu** — yani yanlış bir fiyat bildirildiğinde kimsenin haberi olmuyor ve veri düzelmiyordu.

**Yapılan: `supabase/functions/bildirim-ozet/index.ts`.**

| | karar | gerekçe |
|---|---|---|
| Sıklık | **Her gece, bildirim olmasa da** | Mustafa'nın seçimi: e-postanın GELMEMESİ "bozuldu" işareti olsun. "Bildirim yoksa atla" kısayolu bilerek YOK (testle kilitli) |
| Pencere | **Kayan 24 saat** | Gece işi her gün aynı saatte koşmuyor (GitHub zamanlayıcısı saatlerce gecikebiliyor — bu dosyada ölçülü). Takvim günü olsaydı gecikmeli koşularda bildirimler ya iki kez ya hiç raporlanırdı |
| Alıcı | `OZET_EPOSTA` secret'ı | Depo PUBLIC; adres koda yazılmaz. Secret yoksa **500** (sessiz "başarılı" yok) |
| Gizlilik | E-postada **kullanıcı kimliği yok** | Yalnız "kaç farklı kişi" sayılıyor — uygulamanın kendi kuralının (kaç kişi görünür, kim görünmez) aynısı |
| Kaçış | Ürün adı `kacir()`'den geçiyor | B1 kuralı: kaynak bizim DB'miz olsa da kaçış ÇIKTI tarafında |
| Sınır | 100 satır, **sessiz değil** | Kırpılan sayı hem e-postada hem yanıt gövdesinde |
| Gönderici | `onboarding@resend.dev` | Resend'in dev göndericisi yalnız hesap sahibine teslim ediyor; bu özet zaten ona gidiyor, yani bugün DOĞRU çalışıyor. (Bülten için aynı gönderici bir KUSUR — ayrı iş olarak işaretlendi.) |

**Gece işine bağlantı:** `update-data.yml`'e **ayrı `bildirim-ozet` job'u** (`needs: update`). Gerekçe `fiyat-alarm` ile aynı: yan iş yayın yolunu bloke etmesin ama HARD-FAIL kalsın. `continue-on-error` **yok**; 200 dışında `::error::` + `exit 1`.

> **YAN BULGULAR (bu turda ölçüldü, ayrı iş olarak işaretlendi):** (a) `fiyat-alarm-scan` fallback olarak `data/marketfiyati.json` okuyor; dosya depoda **takip edilmiyor** ve raw URL **404** dönüyor. (b) Haftalık bülten `onboarding@resend.dev`'den gidiyor, yani aboneye ulaşamaz; son koşu `{"gonderilen":0,"not":"abone yok"}`.

**Yeni bekçi `test_bildirim_ozet.mjs` (60 iddia):** kaynak grep'i DEĞİL — fonksiyonun **gerçek kaynağı** `node:vm`'de koşuyor (yalnız `jsr:` import'u sahte `createClient` ile değiştiriliyor), Supabase ve Resend sahte. Kapsam: gizli başlık kapısı (yanlış/eksik/uzun başlık + `CRON_SECRET` tanımsızken kapalı) ve yetkisizde **sıfır yan etki**; bildirim varken içerik; **kontrol grubu** bildirim yokken de gönderim; sessiz başarısızlık yasağı (alıcı yok → 500, Resend hatası → 500, DB hatası → 500, ürün adı gelmezse özet yine gider ama uyarı düşer); kimlik sızıntısı yasağı; çıktı kaçışı; sessiz kırpma yasağı; eksik veride `—` (NaN yok); ve **gece işine gerçekten bağlı mı** (bu depoda kayıtlı boşluk sınıfı: kod yazılır, çağıran olmaz). Prove-by-breaking **11/11**.

> **TESTİN KENDİ KÖR NOKTASI İKİ KEZ ISIRDI, ikisi de düzeltildi (iddia gevşetilmedi):** (1) "CRON_SECRET tanımsız" senaryosunu `secret: undefined` ile kurmuştum — yıkım varsayılanı devreye girip **doğru** secret'ı veriyordu, yani o güvenlik testi hiçbir şey ölçmüyordu; `null` ile ayrıldı. (2) Satır sayımında başlık satırını elle düşüyordum ama desen (`/<tr>/`) başlığı zaten saymıyordu → 100 yerine 99 sayıp yanlış alarm verdi.

**Doğrulama:** 69 `test_*.mjs` + 8 `test_*.py` **yeşil** · prove-by-breaking **11/11** · `update-data.yml` YAML olarak ayrıştırıldı (üç job: `update`, `fiyat-alarm`, `bildirim-ozet`). Fonksiyon **canlıda denenmedi** (deploy Mustafa'da).

**KURULUM (yapıldı 2026-09-16, tekrarı gerekirse):**
1. `OZET_EPOSTA` secret'ı — Dashboard → Edge Functions → **Secrets** (SQL Editor DEĞİL). **Adres, Resend hesabının e-postasıyla AYNI olmalı**: gönderici `onboarding@resend.dev` yalnız hesap sahibine teslim ediyor, başka adreste Resend reddeder → fonksiyon 500 → iş kırmızı.
2. `npx supabase functions deploy bildirim-ozet --project-ref gbgxxahhbfnulmyecxia --use-api` — **depo kökünden** çalıştırılmalı; başka klasörde `Entrypoint path does not exist` 400'ü alınır (bu turda bir kez yaşandı). `--use-api` Docker'ı gereksiz kılıyor. CLI PATH'te yok, `npx` ile koşuyor.
3. Doğrulama: `update-data.yml` → Run workflow; `bildirim-ozet` job'u yeşil, yanıt `{"ok":true,...}`, e-posta geldi mi.

> **KAPI KONTROLÜ (kimlik bilgisi kullanmadan, kontrol gruplu):** `POST /functions/v1/bildirim-ozet` başlıksız → **401** (`UNAUTHORIZED_NO_AUTH_HEADER`), olmayan bir fonksiyon → **404**. Yani 401 "var ve korumalı" demek; 404 "deploy edilmemiş" demek.

---

### 2026-09-14 — Masaüstü Fırsatlar sekme çubuğu okunur oldu (`sw.js` v249 → **v250**)

**Durum: commit edildi, YAYINDA DEĞİL** (push Mustafa'nın kararına). Mustafa masaüstü ekran görüntüsüyle bildirdi: *"web tarafında bunu düzelt beyaz yap renkleri."*

**Kök sebep (ölçüldü, 1440px açık tema):** masaüstü bloğundaki toplu kural `.search-wrap, .cat-search-wrap, …, .firsat-tabs, … { background: transparent; border-bottom: none; }` sekme çubuğunu saydam yapıyor; çubuk ekranın yeşil bandının üstüne düşüyor. Pasif sekme `#6B7280`, aktif `#0E4938` → yeşil üstünde okunmuyor.

**Neden yalnız yazıyı beyaz yapmak YETMEZDİ:** çubuk `position: sticky; top: 53px`. Kaydırınca arkasına bant değil `#F8F9FA` liste zemini geliyor — saydam çubukta beyaz yazı beyaz üstünde kaybolurdu. Çubuk artık **kendi** yeşil zeminini taşıyor (`--zemin-a`, bandın taban rengi), yazı her durumda yeşil üstünde.

| | önce | sonra |
|---|---|---|
| çubuk zemini (masaüstü açık) | saydam | `#0E4938` (kaydırınca da) |
| aktif sekme | `#0E4938` yeşil üstünde | beyaz + beyaz alt çizgi, **10,35** |
| pasif sekme | `#6B7280` yeşil üstünde | %80 beyaz, **7,28** |
| klavye odak halkası | genel `--primary` = çubukla aynı yeşil (görünmez) | beyaz; **yalnız renk + boşluk** değişti, kalınlık genel kuraldan (yüksek kontrastta 3px korunuyor) |

**Değişmeyenler (kontrol grubu, tarayıcıda ölçüldü):** mobil (390px) çubuk beyaz, aktif `#0E4938`, pasif `#6B7280` — aynı · koyu tema çubuk saydam, pasif `rgb(229,231,235)`, aktif `--primary` hap — aynı (koyu için `[data-theme="dark"] .firsat-tabs { background: transparent }` eski durumu birebir koruyor).

**Yeni bekçi `test_firsat_sekme_masaustu.mjs` (21 iddia):** kaynak grep'i değil **basamak çözümü** — `style.css` ayrıştırılıp her özellik için medya sorgusu → özgüllük → sıra ile kazanan bildirim bulunuyor, token çözülüyor, kontrast hesaplanıyor. Alet kontrolü gömülü (kusurun kaynağı olan toplu kuralı ve mobil tabanı görmezse durur). Prove-by-breaking **7/7** (yeşil zemin silindi · pasif %30 · aktif eski yeşil · odak rengi silindi · odak kısayola döndü · koyu override silindi · beyaz renk mobile sızdı). 68 `test_*.mjs` + 8 `test_*.py` yeşil · build yeşil.

---

### 2026-09-13 — Fiyat uyuşmazlığı: kaynak TEK bir İstanbul mağazasını veriyor (`sw.js` v248 → **v249**)

**Durum: commit edildi.** Mustafa bir uyuşmazlık yakaladı: CarrefourSA'nın kendi uygulamasında **449,90 ₺ (kart) / 749,90 ₺**, bizde **399,90 ₺**. İstek: *"muhtemelen başka ürünlerde de vardır, kontrol edilmesi lazım — özellikle verisi paylaşan marketler üzerinde."*

**Kök sebep: ayrıştırma değil KAPSAM.** marketfiyati (TÜBİTAK) her zincir için **tek temsilci mağaza** döndürüyor; `indexTime` bugün 08:50 diyor, yani veri **taze**. Ürün Antalya mağazalarıyla sorulduğunda kaynakta **hiç yok** — kullanıcının gördüğü 449,90/749,90 kaynağa girmemiş.

| ölçüm (2026-09-13) | sonuç |
|---|---|
| Katalogu besleyen mağazalar | **27 mağaza, hepsi İstanbul** (Kadıköy/Üsküdar/Ümraniye/Acıbadem); aynı zincirde format karışık (Carrefour 5.162 kayıt "Hıper", 1.281 "Mını") |
| Canlı düşenler listesi (336 iddia) | 329'u izlendi → **hepsi İstanbul mağazalarından** |
| Genel örneklem (40 ürün, Antalya depolarıyla) | **16'sı (%40) orada kaynakta yok**; bulunan 24'ün **22'sinde fiyat birebir aynı**, 2'sinde %1 |
| **İndirim iddialarında örneklem (25)** | **%12 ürün yok + %20 fiyat farklı → 3'te 1'i geçersiz** (Familia Havlu 99,90 ↔ 174,95; Omo Express 299 ↔ 469; Ülker Hobby 80 ↔ 100) |
| Tazelik | `indexTime` örneklemin **tamamında bugün** → bayatlık sorunu YOK |
| Üstü çizili "eski fiyat" | 2.776 kaydın **749'u (%27)** bizim o markette hiç görmediğimiz bir değer; kaynak o kayıtlarda **`discount:false`** diyor |

> **API'NİN VERİP ATTIĞIMIZ ALANLAR (bu turda çıktı).** `indexTime` (fiyatın kaynakta güncellendiği an), mağaza `latitude/longitude`, ayrıca `unitPrice`, `percentage`, `discount`, `discountRatio`, `promotionText`. Bu deponun tekrar eden deseni: **`marka`, `liste_fiyat`, `depot_id` de aynı şekilde bulunmuştu** — "API ne veriyor" sorusunu düzenli sormak gerekiyor.

**YAPILAN (D + A).**
- **D1 — `fiyat_guncelleme` + `depot_il` (additive).** `parse_product` artık `indexTime`'ı ISO'ya çevirip yazıyor ve mağaza koordinatından ili türetiyor. İl tablosu **`iller_tablo.py`'ye TEK KAYNAK olarak çıkarıldı** (`il_market_tara.py` de oradan okuyor; kopya = kaçınılmaz sapma). Uzak/geçersiz koordinatta alan **hiç açılmıyor** — yanlış il yazmaktansa hiç yazma.
- **D2 — ilan rozeti artık iddia etmiyor.** `listeFiyatHTML` ikinci argüman olarak o markette gördüğümüz **en yüksek fiyatı** alıyor; ilan bunun üstündeyse "-%N" rozeti **basılmıyor**, yerine "ilan · biz görmedik" notu çıkıyor. İlan **silinmiyor** (marketin beyanı) — Mercek A1'in dil kuralı. Geçmiş henüz inmediyse `null` geçiyor: **bilmediğimizi iddiaya çevirmiyoruz**.
- **A — "bu fiyat nereden?"** Ürün detayında market satırının altında mağaza · il · güncellenme saati (`.detay-mkt-kaynak`); ana sayfadaki tazelik satırında **veriden türeyen** kapsam cümlesi ("· 27 mağazadan derlendi"). **Sabit şehir adı yazılmıyor** — `fiyat_kapsami` build'de `depot_id`/`depot_il`'den hesaplanıyor, yarın başka il gelirse cümle kendiliğinden değişir (zam sekmesindeki "sabit ay yazma" kuralının aynısı). Bugün il listesi **boş** (alan yarınki gece koşusuyla dolacak), o yüzden cümle yalnız mağaza sayısını söylüyor.

> **SABİT PENCERE TUZAĞI BİR KEZ DAHA ISIRDI.** `openDetay`'e eklediğim satırlar üç testin (al_zamani, esit_fiyat, supheli) **sabit karakter penceresinden** aranan çağrıları dışarı itti (ölçüldü: çağrılar 4299-4619, pencereler 4000/4500). Testler **gevşetilmedi**: market satırı üreticisi `_detayMarketSatirlariHTML` olarak fonksiyonun **dışına** alındı (openDetay ~720 karakter kısaldı), gözlem haritası `_marketEnYuksekHaritasi`'na çıktı ve `test_esit_fiyat`'ın şablon iddiası üreticinin gövdesine bağlandı + "openDetay ona bağlı" kilidi eklendi.

**AÇIK MADDE — B: "her şehrin fiyatını göster" (Mustafa'nın önceliği).** Ölçülmeden koda geçilmeyecek; gereken ölçümler:
1. **Bölge sayısı:** 81 il gerekmiyor olabilir — fiyatlar ülke geneli büyük ölçüde aynı çıktı (22/24). Önce "kaç farklı fiyat bölgesi var" ölçülmeli (birkaç il × birkaç zincir, aynı ürün sepeti).
2. **İstek/süre maliyeti:** `/api/v2/search` depo listesiyle çalışıyor; N bölge × 16 bin ürün gece koşusunu ne kadar uzatır (bugün ~20 dk).
3. **Dosya boyutu:** bölge başına fiyat taşımak `urunler_*.json` ve `gecmis_fiyatlar.json`'u kaç katına çıkarır (bugün 14 MB ham / 653 KB gzip geçmiş).
4. **Kapsama:** örneklemde ürünlerin %40'ı Antalya'da kaynakta YOK — bölgesel veri "eksik ürün" sorununu çözmüyor, o boşluk marketlerden veri almadan kapanmaz (**Mustafa: gerekirse marketlerle iletişime geçilecek**).
5. **Ürün kararı:** şehir seçimi bugün yalnız zincir mevcudiyetini etkiliyor; fiyatlara bağlanınca Profil'deki cümle ve tüm "en ucuz" iddiaları o şehre göre yeniden tanımlanır.

> **ÖLÇÜLDÜ (2026-09-13) — B ERTELENDİ, ÖNCELİK MARKET VERİSİ (karar Mustafa'nın).** Aynı 8 il (İstanbul, Ankara, İzmir, Antalya, Alanya, Gaziantep, Trabzon, Erzurum), iki sepet:
>
> | sepet | sonuç |
> |---|---|
> | yaygın ürün (12, en çok markette satılan) | **9/12'sinde 8 ilin hepsinde fiyat aynı**; farklı olan 3'ünde fark ≤%9 (58↔59, 32↔34, 31↔34); hiç eksik yok |
> | düşenler (12 indirim iddiası) | 96 iddia-il kombinasyonunun **63'ünde (%66) fiyatımız geçerli** (İstanbul dışı %63). Başarısızlığın **28/33'ü "ürün o ilde kaynakta yok"**, yalnız 5'i fiyat farkı. Erzurum **3/12**, Gaziantep 6/12 geçerli |
>
> **Sonuç:** bölgesel çekim fiyat doğruluğunu pek değiştirmiyor (ürün o ilde raporlanıyorsa fiyat çoğunlukla aynı); asıl boşluk o ildeki **ürün kaydının kendisi** ve bu ancak **marketlerden doğrudan veriyle** kapanır. **Dürüst sınır:** "yok" = tam aynı başlıkla bulunamadı; bazı illerde ürün farklı adla listelenmiş olabilir (Familia elle sondada Alanya Migros 174,95 göründü, sıkı eşleşmede "yok" çıktı) — gevşek eşleşmeyle yeniden ölçüm yapılmadı, gerekirse ilk iş o. Ölçüm betikleri tek seferlikti (scratchpad), depoya alınmadı.

**Doğrulama:** 67 `test_*.mjs` + 8 `test_*.py` **yeşil** · prove-by-breaking **13/13** (scraper alanları 3, ilan rozeti 5, kapsam cümlesi 5) · build yeşil (`fiyat_kapsami: {magaza: 27, iller: []}`) · yerel `dist`'te tarayıcı.

---

### 2026-09-12 — Şerit 12 kart + "Tümünü gör" + Fırsatlar'da 4. sekme (`sw.js` v247 → **v248**)

**Durum: commit edildi, YAYINDA DEĞİL** (push Mustafa'nın kararına bırakıldı). İstek: *"şeridi büyüt 12 ürün olsun ama yanına bir yere tümünü gör butonu koy, sonra kalan bütün ürünleri Fırsatlar'a sekme olarak oluştur, kullanıcı tümünü gör deyince oraya yönlendirsin."* Sebebi: ölçüt yeniden kurulduktan sonra havuzda **336 gerçek haftalık indirim** vardı, şerit bunun **6'sını** gösteriyordu.

**Yapılanlar.** `DUSENLER_KART` 6 → **12**; şerit başlığına `data-firsat-sekme="dusen"` taşıyan **"Tümünü gör"** düğmesi (satır içi handler YOK, `document` üzerinde tek dinleyici — sayaç kilidi); Fırsatlar'a **4. sekme** (`data-tab="dusen"`, mevcut delegasyon). Sekme listesi build'de **şeritle AYNI havuzdan** üretiliyor: havuz bir kez hesaplanıyor (`__dusenHavuz`), seçim `anasayfa.json`'a, **tamamı** `data/dusenler.json`'a gidiyor — ölçüt, şüphe elemesi ve kayıt biçimi (`dusenKayit`) tek yerde, burada yeni bir "düşüş nedir" tanımı yok.

| | ölçüm |
|---|---|
| market sınırı 2 → **3** | 12 kartın sonuncusu ≤2'de havuzun **106. sırasından** (-%25), ≤3'te **50. sırasından** (-%31) geliyordu; ≤4'te 12 kartın 8'i iki zincirden (çeşitlilik kaybı) |
| `data/dusenler.json` | 336 ürün, 142,5 KB ham / **23,3 KB gzip** — **tembel** (yalnız sekme açılınca), `sw.js` önbelleğinde **değil**, `.gitignore`'da (saf build çıktısı) |
| ana sayfa maliyeti | **değişmedi** — şerit tam listeyi indirmiyor (testle kilitli) |

- **Rozet kademesi tek sabitte** (`DUSEN_BUYUK_YUZDE = 25`): aynı ürün şeritte ve sekmede aynı renkte. Yeni renk tanımlanmadı — mevcut `--rozet-iyi-*` / `--rozet-supheli-*` aileleri (koyu tema dahil).
- **Tam liste inmezse** şeritteki kartlar + "Tam liste yüklenemedi" notu gösteriliyor; 14 MB katalog indirip istemcide yeniden hesaplama **yok** (şeridin kuralıyla aynı). Hata **önbelleğe yazılmıyor** (boş dizi "yüklendi" sayılırdı — `catCache`/arama indeksinde yaşanan kusur).
- **Üç yarış kapısı**: geç gelen "En Ucuz" / "Zamlananlar" / düşenler cevabı, kullanıcı başka sekmeye geçtiyse onun içeriğini **ezmiyor**. Sekme değişince özet çipi de anında siliniyor.

> **SALT OKUNUR DÜŞMANCA İNCELEME (17 ajan; 20 bulgu → 6 doğrulama → 5 AYAKTA, 1 çürütüldü).** Beşi de düzeltildi:
>
> | | Bulgu | Neden önemli | Düzeltme |
> |---|---|---|---|
> | a | Veri hattında **"ölçülemedi" ile "ölçüldü, sıfır" ayrımı yoktu** | Build'de Supabase çökerse havuz boş yazılıyor; emniyet kapısı **dört şeridin birden** boş olmasını istediği için hiç tetiklenmiyor (zam/tuzaklar yerel veriden dolu) → deploy yeşil. Şerit kendini gizliyordu ama **yeni sekme gizlenmiyor** ve "0 Düşen ürün" diye **ölçülmüş gibi duran** bir iddia basıyordu | `dusenler.json` artık `olculdu` bayrağı taşıyor (build'de `_puanCache`'ten); ölçülemediyse sayı **yazılmıyor**, cümle "şu an ölçülemedi" |
> | b | Kartta **üç kaynak**: rozet ve "normal" anlık görüntüden, indirimli fiyat **canlı** katalogdan | `catCache` oturum boyunca yapışık (hiç boşaltılmıyor) → bayat fiyatla "-%60" derken alt metin "249,90 ₺ → 249,90 ₺" diyebiliyordu. Rozetin abartması bu deponun **en pahalı hata sınıfı** (Dalin "-%51" → gerçek %11) | `dusenKayit`'e `fiyat` eklendi; kart, rozet ve okun iki ucu **tek kayıttan** |
> | c | Kart **kararsız `_id`** ile çözülüyordu | `_id` = `slug_index`; veri her gece yeniden dizilince aynı index **başka ürüne** denk gelir (sepet tarafında aynı sınıf `_sid` ile çözülmüştü) | productMap araması kalktı — çizim anlık görüntüden |
> | d | Özet çipi sekme değişince temizlenmiyordu | "En Ucuz" sekmesinde cevap gelene kadar **"336 Düşen ürün"** yazıyordu: yanlış **etiketli** sayı | `renderFirsatlar` başında özet de sıfırlanıyor |
> | e | `_firsatSekmeGorunur` testte **sessizce ölüydü** | Sahte DOM `offsetLeft`/`clientWidth` taşımadığı için yardımcı erken dönüyordu; yani hiç ölçülmüyordu | Sahte DOM ölçülebilir yapıldı + kontrol grubu (sekme görünüyorsa çubuk oynamamalı) |
>
> Çürütülen: *"'Tümünü gör' odak yönetimi yok"* (ekran geçişlerinin tamamı aynı desende; bu işe özgü bir kusur değil). **Kotaya sığmayan 14 bulgu doğrulanmadan kaldı** — en dikkate değerleri aşağıda "bilerek yapılmayan"da.

> **BOZMA ARACININ KENDİSİ İKİ KEZ YANILTTI (bu turun alet dersi).** (1) Planda `"test": "node test_dusenler.mjs"` yazdım; araç dosya adı bekliyor ve başına kendisi `node` koyuyor → var olmayan bir dosyayı çalıştırıp **"test zaten KIRMIZI"** dedi. Test hiç kırmızı değildi; mesaj doğal okunduğu için iki tur kaybettirdi. (2) İki çapa **tekil değildi** (`const u = x.u;` app.js'te iki yerde): `String.replace` **ilk** eşleşmeyi değiştirdiği için mutasyon **yanlış fonksiyona** uygulandı ve "guard kör" gibi okundu. Harness "0 eşleşme"yi HARD-FAIL ediyor ama **">1 eşleşme"yi yalnızca raporluyor**. Artık plan koşulmadan önce çapaları sayan bir ön kontrol var (scratchpad `capa_dogrula.mjs`) — *aynı ders bu depoda dördüncü kez: aleti teste koymadan önce onun da kontrol grubunu koş.*

> **BİR GUARD KÖRLÜĞÜ GERÇEKTİ ve iddia SIKILAŞTIRILDI:** "kart canlı üründen çizilmesin" iddiam ham metinde `249,90` arıyordu; `tlHTML` fiyatı **parçalı** basıyor (`fp-l` / `fp-k` / `fp-tl`), yani düz dize hiç eşleşmiyordu. İddia kartın kendi fiyat markup'ına bağlandı. Ayrıca **test verisinin kendisi çelişkiliydi** (normal=100, fiyat=70 sabitken yüzde 30→16): 15 kartın 14'ü çelişkili çiziliyordu ve test görmüyordu → veri kendi içinde tutarlı hale getirildi ve **her kartta `rozet = (normal − fiyat) / normal`** diye bağlayıcı bir iddia eklendi.

**Bilerek YAPILMAYAN (not düşülüyor):** (a) **Build kapısı sertleştirilmedi** — şüphe puanları alınamazsa deploy hâlâ yeşil geçiyor (yalnız `::warning` + artık dürüst istemci metni). `exit(1)` yapmak kısmi bir dış arızada tüm yayını durdurur; bu bir **ürün/altyapı kararı**, Mustafa versin. (b) Sekmede **336 kart tek seferde** çiziliyor (üst sınır/sayfalama yok) — ölçüldü: çizim ve kaydırma sorunsuz, ama havuz büyürse tekrar bakılmalı. (c) Sekme çubuğunda `aria-selected` yok (mevcut üç sekmede de yoktu — ayrı iş). (d) Şerit `anasayfa.json`'dan (sw önbellekli), sekme `dusenler.json`'dan (ağ) besleniyor; iki dosyanın damgası karşılaştırılmıyor — açık duran sekme `DATA_UPDATED`'de yeniden çizilmiyor.

**Doğrulama:** `test_dusenler.mjs` 136 → **198 iddia** (TDD: önce 21 kırmızı) · prove-by-breaking **31/31** · 66 `test_*.mjs` + 8 `test_*.py` **yeşil** · build yeşil · yerel `dist`'te gerçek tarayıcı: şerit **12 kart**, "Tümünü gör" → sekme aktif ve **336 kart**, **336 kartın 336'sında rozet = (normal − fiyat)/normal ve kart fiyatı okun sağ ucuyla aynı**, özet "336 Düşen ürün", karta dokununca doğru detay, geri tuşu sekmeyi koruyor, `dusenler.json` **tek istek** (ana sayfada hiç inmiyor), 375px'te sekme çubuğu 140px kaydırılıp 4. sekme görünür oluyor, yatay taşma 0, konsol hatası **0**.
...
| `test_dusenler.mjs` | 68 | "Bu hafta düşenler": 60 günlük normal fiyat (üst ortanca), son 7 günde başlayan ve süren indirim, ilan yolu (gözlemle iki yönlü uyuşma), en ucuz market = kart fiyatı, şüphe elemesi, çeşitlilik (marka/kategori/market), hazır liste / geriye düşüş çizimi, gerçek veri değişmezleri. test_supheli'nin iki düşenler testi buraya taşındı |
...
| `test_bildirim_ozet.mjs` | 60 | Gecelik "bu fiyat tutmadı" özet e-postası: fonksiyonun gerçek kaynağı `node:vm`'de sahte Supabase + sahte Resend ile koşuyor. Gizli başlık kapısı ve yetkisizde sıfır yan etki · **kontrol grubu: bildirim yokken de gönderiliyor** (Mustafa'nın kararı) · sessiz başarısızlık yasağı (alıcı yok / Resend hatası / DB hatası → 500) · kullanıcı kimliği e-postaya girmiyor · ürün adı kaçışlı · kayan 24 saat penceresi · sessiz kırpma yasağı · eksik veride NaN yok · **gece işine gerçekten bağlı mı** (`update-data.yml`: job var, gizli başlık gidiyor, `continue-on-error` yok, 200 dışında exit 1) |
...
| `test_firsat_sekme_masaustu.mjs` | 21 | Masaüstü Fırsatlar sekme çubuğu: `style.css` ayrıştırılıp **basamak çözülüyor** (medya → özgüllük → sıra), token'lar çözülüp kontrast hesaplanıyor. Açık masaüstünde çubuk zemini opak (sticky), pasif/aktif ≥4,5, alt çizgi ve odak halkası ≥3, odak kuralı kalınlığı ezmiyor · kontrol grubu: mobil ve koyu tema değişmedi · yeni kurallar yalnız ≥1024px'te. Alet kontrolü gömülü |
...
- **Ana sayfa "Bu hafta düşenler"** → ~~`get_fiyat_dusenler()` RPC~~ **2026-09-11'den beri app.js'te hesaplanıyor** (`dusenHavuzu`; RPC emekli — bkz. 2026-09-11 bloğu)
...
RPC fonksiyonları: `get_fiyat_dusenler(p_limit)` (**2026-09-11'den beri KULLANILMIYOR**, DB'de duruyor), `indirim_puan_toplu_guncelle(guncellemeler jsonb)`, `get_fiyat_bildirimleri()`, `get_kendi_bildirim_sayim()` (2026-08-08, security definer; anon'a kapalı — profil "Katkılarım" bölümü bunu kullanır).
...
- **Supabase** — auth, DB, Edge Functions, RPC (`get_fiyat_dusenler` — 2026-09-11'den beri kullanılmıyor, `indirim_puan_toplu_guncelle`, `get_fiyat_bildirimleri`, `jsonb_fiyat_max`)
