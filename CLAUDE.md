...
| `test_gecis.mjs` | 41 | Ekran/sekme geçişi + splash çıkışı: **%100 öteleme yasağı** (boş ekran üretiyordu — ilk kare kapsamı %0), öteleme ≤32px, süre tek kaynak (`--gecis-ekran`) ve iki yönde **aynı**, splash sönme eğrisinin **öne yüklü olmadığı SAYIYLA** doğrulanıyor (bezier çözülüp "sürenin %25inde ilerleme ≤%40") — string eşleşmesi değil, başka bir öne yüklü eğri de kırmızı verir · **prefers-reduced-motion**'da açık `animation: none` · keyframe'de yerleşim özelliği yasağı · `showScreen`in sınıfı hâlâ eklediği + `void offsetWidth` reflow'u · satır içi stil yasağı. Kendi ölçüsünü doğrulayan alet kontrolü gömülü |
